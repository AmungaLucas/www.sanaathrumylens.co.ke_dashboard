...
export async function GET(request) {
    ...
    let sql = `
        SELECT l.created_at, a.name as actor_name, l.action, l.entity_type, l.entity_id, l.old_values, l.new_values, l.ip_address
        FROM audit_logs l
        LEFT JOIN admin_users a ON l.actor_id = a.id
        WHERE 1=1
    `;
    const params = [];

    if (search) {
        sql += ` AND (a.name LIKE ? OR l.action LIKE ? OR l.entity_type LIKE ?)`;
