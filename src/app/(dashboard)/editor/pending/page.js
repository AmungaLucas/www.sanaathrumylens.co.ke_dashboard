...
import { Clock, Eye, CheckCircle, Search } from 'lucide-react';
...
export default function EditorPendingPage() {
    ...
    const fetchPosts = async () => {
        try {
            const res = await fetch(`/api/dashboard/editor/pending?status=${filter}`);
        ...
        <div>
            <div className="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p className="text-gray-600 mt-1">Content awaiting editorial review</p>
                </div>
            ...
            <div className="mb-6">
                <div className="relative">
                    ...
                    <input type="text" placeholder="Search by title or author..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg" />
                </div>
            </div>
                            ...
                            {filteredPosts.map((post) => (
                                <tr key={post.id} className="border-b border-gray-100 hover:bg-gray-50">
                                    ...
                                    <td className="text-center py-3 px-6">
                                        <button onClick={() => router.push(`/editor/queue/${post.id}/review`)} className="px-3 py-1 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700">
                                            Review
                                        </button>
                                    </td>
                                </tr>
