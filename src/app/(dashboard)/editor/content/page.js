...
import { Search, Eye, Edit, CheckCircle, Trash2 } from 'lucide-react';
...
export default function EditorContentPage() {
    ...
    const fetchContent = async () => {
        try {
            const res = await fetch(`/api/dashboard/admin/content?type=blog&search=${search}&status=${status}&page=${page}`);
        ...
        <div>
            <div className="flex justify-between items-center mb-6">
                <div>
                    ...
                    <p className="text-gray-600">Browse and review all content</p>
                </div>
            </div>
                ...
                <div className="flex flex-wrap gap-4">
                    <form onSubmit={handleSearch} className="flex-1">
                        <div className="relative">
                            ...
                            <input type="text" placeholder="Search content..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-10 pr-4 py-2 border rounded-lg" />
                        </div>
                    </form>
                                ...
                                <td className="text-center py-3 px-6">
                                    <div className="flex justify-center gap-2">
                                        {item.status === 'PENDING_REVIEW' && (
                                            <Link href={`/editor/queue/${item.id}/review`} className="px-3 py-1 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700">Review</Link>
                                        )}
                                    </div>
                                </td>
                            </tr>
